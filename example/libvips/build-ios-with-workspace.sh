...
#   --clean         清理所有构建产物后重新编译
#   --skip-compile  跳过编译，只创建 XCFramework
#   --help          显示帮助信息
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --skip-compile  跳过编译，只创建 XCFramework"
            echo "  --help          显示帮助信息"
            echo ""
            echo "示例:"
            ...
            echo "  ./build_with_workspace.sh --skip-compile     # 只创建 XCFramework"
            exit 0
            ;;
        *)
            echo "未知选项: $1"
...
if [ "$SKIP_COMPILE" = false ]; then
    # 构建 workspace 命令参数
    WORKSPACE_ARGS="workspace -C example/libvips -i --no-android"
...
# 构建 package xcframework 命令参数
XCF_ARGS="package xcframework -C example/libvips"
XCF_ARGS="$XCF_ARGS -p $INSTALL_PREFIX"
XCF_ARGS="$XCF_ARGS -t libvips -o libvips"

dart run bin/compile.dart $XCF_ARGS
